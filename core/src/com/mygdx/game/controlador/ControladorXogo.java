...
public class ControladorXogo {
    ...
    private void controladorSerpiente(float delta){
        ...
        for (int i=0;i<serpiente.getAneis().size;i++) {
            ...
            if (serpiente.getAneis().get(i).getPosicion().x <= 0) {
                //serpiente.getAneis().get(0).setPosicion(0, serpiente.getPosicion().y);
                serpiente.iniciarAneis();
                //morre a serpiente
            } else {
                if (serpiente.getAneis().get(i).getPosicion().x >= Mundo.TAMANO_MUNDO_ANCHO
                        - serpiente.getAneis().get(i).getTamano().x) {
                    serpiente.getAneis().get(i).setPosicion(Mundo.TAMANO_MUNDO_ANCHO
                            - serpiente.getAneis().get(i).getTamano().x, serpiente.getAneis().get(0).getPosicion().y);
                }
            }

            //para que no se vaya infinitamente hacia arriba o abajo
            if (serpiente.getAneis().get(i).getPosicion().y <= Controis.FONDO_NEGRO.height) {
                serpiente.getAneis().get(i).setPosicion(serpiente.getAneis().get(i).getPosicion().x,
                        Controis.FONDO_NEGRO.height);
            } else {
                if (serpiente.getAneis().get(i).getPosicion().y >= Mundo.TAMANO_MUNDO_ALTO
                        - serpiente.getAneis().get(i).getTamano().y) {
                    serpiente.getAneis().get(i).setPosicion(serpiente.getAneis().get(i).getPosicion().x,
        ...
        */
            if (Intersector.overlaps(meuMundo.getManzana().getRectangulo(), serpiente.getAneis().get(0).getRectangulo())){
